...
DISABLE_ALL_EMAIL char(1) not null,
...
ENABLED char(1) not null,
ALLOWED_TO_ACTIVATE_BUILD char(1) not null,
...
ENABLED char(1) not null,
SERIALIZE char(1) not null,
DELETED char(1) not null,
CAPACITY integer not null,
MAX_CONCURRENT_BUILDS integer not null,
TIMESTAMP bigint not null,
...
constraint CLUSTER_MEMBER_UC2 unique (CLUSTER_ID, AGENT_ID),
constraint CLUSTER_MEMBER_FC1 foreign key (AGENT_ID) references AGENT(ID) ON DELETE CASCADE,
...
constraint BUILD_CONFIG_FC1 foreign key (BUILDER_ID) references CLUSTER(ID) ON DELETE CASCADE
);
...
constraint MANUAL_RUN_PARAMETER_ATTRIBUTE_UC2 unique (TYPE, BUILD_ID, NAME)
);
...
PATH varchar(1024) not null,
TYPE tinyint not null,
FAIL_IF_NOT_FOUND char(1) not null,
IGNORE_TIMESTAMP char(1) not null,
SHELL_VARIABLE varchar(200) not null,
TIMESTAMP bigint not null,
...
constraint TEST_CASE_UC2 unique (TEST_PACKAGE_ID, NAME),
...
insert into SYSTEM_PROPERTY (ID, NAME, VALUE, TIMESTAMP) values (5, 'parabuild.admin.name', 'Build Administrator', 1);
insert into SYSTEM_PROPERTY (ID, NAME, VALUE, TIMESTAMP) values (6, 'show.details.on.dashboard', 'checked', 1);
insert into SYSTEM_PROPERTY (ID, NAME, VALUE, TIMESTAMP) values (7, 'round.robin.load.balancing', 'checked', 1);
insert into SYSTEM_PROPERTY (ID, NAME, VALUE, TIMESTAMP) values (8, 'respect.intermediate.step.failure', 'checked', 1);
insert into SYSTEM_PROPERTY (ID, NAME, VALUE, TIMESTAMP) values (9, 'parabuild.enable.advanced.build.settings', 'checked', 1);
insert into SYSTEM_PROPERTY (ID, NAME, VALUE, TIMESTAMP) values (10, 'use.git.user.email', 'checked', 1);
insert into GROUPS (ID, NAME, DESCR, ENABLED, ALLOWED_TO_CREATE_BUILD, ALLOWED_TO_DELETE_BUILD, ALLOWED_TO_START_BUILD, ALLOWED_TO_STOP_BUILD, ALLOWED_TO_UPDATE_BUILD, ALLOWED_TO_VIEW_BUILD, ALLOWED_TO_PUBLISH_RESULTS, ALLOWED_TO_DELETE_RESULTS, ALLOWED_TO_CREATE_RESULT_GROUP, ALLOWED_TO_DELETE_RESULT_GROUP, ALLOWED_TO_UPDATE_RESULT_GROUP, ALLOWED_TO_VIEW_RESULT_GROUP, ALLOWED_TO_ACTIVATE_BUILD, TIMESTAMP) values (0, 'Anonymous', 'Users that are not logged in', 'Y', 'N', 'N', 'N', 'N', 'N', 'Y', 'N', 'N', 'N', 'N', 'N', 'Y', 'N', 0);
...
insert into AGENT (ID, HOST, DESCRIPTION, ENABLED, SERIALIZE, CAPACITY, MAX_CONCURRENT_BUILDS, DELETED, TIMESTAMP) values (0, '<Build Manager>', 'Agent that runs on the build manager machine', 'Y', 'N', 1, 0, 'N', 0);
insert into CLUSTER (ID, NAME, DESCR, ENABLED, DELETED, TIMESTAMP) values (0, 'Local Builder', 'Single-agent build farm that runs builds on the build manager machine', 'Y', 'N', 0);
