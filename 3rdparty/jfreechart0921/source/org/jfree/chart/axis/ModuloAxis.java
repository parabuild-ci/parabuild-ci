 ...
 * Contributor(s):   -;
 *
 * $Id: ModuloAxis.java,v 1.1 2004/08/31 14:30:23 mungady Exp $
 *
 * Changes
 * -------
...
public class ModuloAxis extends NumberAxis {
    ...
    private double trans(double value, Rectangle2D area, RectangleEdge edge) {
        ...
        if (isInverted()) {
            return max - ((value - this.displayStart) / (this.displayEnd - this.displayStart)) 
                * (max - min);
        }
        else {
            return min + ((value - this.displayStart) / (this.displayEnd - this.displayStart)) 
                * (max - min);
        }
