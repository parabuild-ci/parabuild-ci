 ...
 * Contributor(s):   -;
 *
 * $Id: CSV.java,v 1.2 2004/09/08 09:17:16 mungady Exp $
 *
 * Changes
 * -------
...
/**
 * A utility class for reading {@link CategoryDataset} data from a CSV file.  This initial version
 * is very basic, and won't handle errors in the data file very gracefully.
 */
public class CSV {
    ...
    /** 
     * Creates a new CSV reader where the field delimiter is a comma, and the text delimiter is 
     * a double-quote.
     */
    public CSV() {
        this(',', '"');    
     ...
     * 
     * @param fieldDelimiter  the field delimiter (usually a comma, semi-colon, colon, tab or 
     *        space).
     * @param textDelimiter  the text delimiter (usually a single or double quote).
     */
    ...
    private List extractColumnKeys(String line) {
        ...
        for (int i = 0; i < line.length(); i++) {
            if (line.charAt(i) == this.fieldDelimiter) {
                if (fieldIndex > 0) {  // first field is ignored, since column 0 is for row keys
