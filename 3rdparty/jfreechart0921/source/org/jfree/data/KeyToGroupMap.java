 ...
 * Contributor(s):   -;
 *
 * $Id: KeyToGroupMap.java,v 1.1 2004/08/31 15:21:49 mungady Exp $
 *
 * Changes
 * -------
 * 29-Apr-2004 : Version 1 (DG);
 * 07-Jul-2004 : Added a group list to ensure group index is consistent, fixed cloning 
 *               problem (DG);
 * 
 */
...
public class KeyToGroupMap implements Cloneable, PublicCloneable, Serializable {
    ...
    /**
     * Returns a list of the groups (always including the default group) in the map.  The
     * returned list is independent of the map, so altering the list will have no effect.
     * 
     ...
     * 
     * @return The group index (or -1 if the group is not represented within the map).
     */
     ...
     * 
     * @param key  the key (<code>null</code> not permitted).
     * @param group  the group (<code>null</code> permitted, clears any existing mapping).
     */
     ...
     * always return an accurate result for the default group, since explicit mappings 
