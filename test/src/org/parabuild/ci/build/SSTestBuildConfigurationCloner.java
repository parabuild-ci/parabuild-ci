...
public class SSTestBuildConfigurationCloner extends ServersideTestCase {
  ...
  public void test_createBuildRunConfigTakesInAccountOwerwrites() throws Exception {
    ...
    overwriteList.add(new SourceControlSettingVO(VersionControlSystem.CVS_BRANCH_NAME, TEST_SOME_NEW_BRANCH));
    // run
    ...
    // assert
    final SourceControlSetting sourceControlSetting = cm.getSourceControlSetting(newBuildConfig.getBuildID(), VersionControlSystem.CVS_BRANCH_NAME);
  ...
  public void test_createBuildRunConfig_Bug777() throws Exception {
    ...
    final SourceControlSetting scs = (SourceControlSetting) effectiveSourceControlSettingsAsMap.get(VersionControlSystem.P4_DEPOT_PATH);
    ...
    final SourceControlSetting newScs = (SourceControlSetting) newEffectiveSourceControlSettingsAsMap.get(VersionControlSystem.P4_DEPOT_PATH);
  ...
  public void test_createReferenceBuildRunConfig() throws Exception {
    ...
    final SourceControlSetting sourceControlSetting = cm.getSourceControlSetting(newBuildConfig.getBuildID(), VersionControlSystem.REFERENCE_BUILD_ID);
    ...
    final int activeBackingBuildConfigID = cm.getSourceControlSetting(TEST_REFERENCE_BUILD_ID_TO_CLONE,
            VersionControlSystem.REFERENCE_BUILD_ID).getPropertyValueAsInt();
    // assert
  ...
  public void test_createActiveBuildClearCaseVWSStorageIsNotCopied() throws Exception {
    ...
    try {
      ...
      cm.save(new SourceControlSetting(testClearcaseBuildId, VersionControlSystem.CLEARCASE_VIEW_STORAGE_LOCATION_CODE, VersionControlSystem.CLEARCASE_STORAGE_CODE_VWS));
      cm.save(new SourceControlSetting(testClearcaseBuildId, VersionControlSystem.CLEARCASE_VIEW_STORAGE_LOCATION, "test_view_storage_path"));
      newBuildConfig = cloner.createActiveBuildConfig(testClearcaseBuildId);
      assertNull(cm.getSourceControlSetting(newBuildConfig.getActiveBuildID(), VersionControlSystem.CLEARCASE_VIEW_STORAGE_LOCATION_CODE));
      assertNull(cm.getSourceControlSetting(newBuildConfig.getActiveBuildID(), VersionControlSystem.CLEARCASE_VIEW_STORAGE_LOCATION));
    } finally {
  ...
  public void test_createActiveBuildClearCaseStglocStorageIsCopied() throws Exception {
    ...
    try {
      final int testClearcaseBuildId = TestHelper.TEST_CLEARCASE_VALID_BUILD_ID;
      cm.save(new SourceControlSetting(testClearcaseBuildId, VersionControlSystem.CLEARCASE_VIEW_STORAGE_LOCATION_CODE, VersionControlSystem.CLEARCASE_STORAGE_CODE_STGLOC));
      cm.save(new SourceControlSetting(testClearcaseBuildId, VersionControlSystem.CLEARCASE_VIEW_STORAGE_LOCATION, "test_view_storage_name"));
      newBuildConfig = cloner.createActiveBuildConfig(testClearcaseBuildId);
      assertNotNull(cm.getSourceControlSetting(newBuildConfig.getActiveBuildID(), VersionControlSystem.CLEARCASE_VIEW_STORAGE_LOCATION_CODE));
      assertNotNull(cm.getSourceControlSetting(newBuildConfig.getActiveBuildID(), VersionControlSystem.CLEARCASE_VIEW_STORAGE_LOCATION));
    } finally {
      if (newBuildConfig != null) {
        buildListService.removeBuild(newBuildConfig.getBuildID());
      }
