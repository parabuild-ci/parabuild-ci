...
public class SSTestRepositoryBrowserPanel extends ServletTestCase {
  ...
  public void test_loadFishEye() throws Exception {
    TestHelper.setSourceControlProperty(TEST_CVS_VALID_BUILD_ID, SourceControlSetting.REPOSITORY_BROWSER_TYPE, Integer.toString(SourceControlSetting.CODE_FISHEYE));
    TestHelper.setSourceControlProperty(TEST_CVS_VALID_BUILD_ID, SourceControlSetting.FISHEYE_ROOT, "fish_eye_root");
    TestHelper.setSourceControlProperty(TEST_CVS_VALID_BUILD_ID, SourceControlSetting.FISHEYE_URL, "http://fish_eye_url");
  ...
  public void test_loadViewVC() throws Exception {
    TestHelper.setSourceControlProperty(TEST_CVS_VALID_BUILD_ID, SourceControlSetting.REPOSITORY_BROWSER_TYPE, Integer.toString(SourceControlSetting.CODE_VIEWVC));
    TestHelper.setSourceControlProperty(TEST_CVS_VALID_BUILD_ID, SourceControlSetting.VIEWCVS_ROOT, "viewvc_root");
    TestHelper.setSourceControlProperty(TEST_CVS_VALID_BUILD_ID, SourceControlSetting.VIEWCVS_URL, "http://viewvc_url");
    final BuildConfig buildConfig = ConfigurationManager.getInstance().getActiveBuildConfig(TEST_CVS_VALID_BUILD_ID);
    panel.load(buildConfig);
    assertEquals(TEST_CVS_VALID_BUILD_ID, panel.getBuildID());
  }
  ...
  public void test_loadNotSelected() throws Exception {
    TestHelper.setSourceControlProperty(TEST_CVS_VALID_BUILD_ID, SourceControlSetting.REPOSITORY_BROWSER_TYPE, Integer.toString(SourceControlSetting.CODE_NOT_SELECTED));
    final BuildConfig buildConfig = ConfigurationManager.getInstance().getActiveBuildConfig(TEST_CVS_VALID_BUILD_ID);
    panel.load(buildConfig);
    assertEquals(TEST_CVS_VALID_BUILD_ID, panel.getBuildID());
  }
