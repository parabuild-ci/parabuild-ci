 ...
 */
abstract class MercurialCommand extends VersionControlRemoteCommand {
   ...
   */
  private static final Logger LOG = Logger.getLogger(MercurialCommand.class); // NOPMD

  private final String exePath;


  protected MercurialCommand(final Agent agent, final String exePath) throws AgentFailureException, IOException {
    super(agent, true);
    ...
    // Set params
    this.exePath = ArgumentValidator.validateArgumentNotBlank(exePath, "Path to hg executable").trim();
   ...
   *
   * @return the arguments to pass to bzr executable including bzr command and it args.
   * @see #preExecute()
   */
  ...
  protected final void postExecute(final int resultCode) throws IOException {
    ...
    try {
      if (resultCode != 0 && getStderrFile().exists() && getStderrFile().length() > 0) {
        ...
        throw new IOException("Errors while executing hg command: " + message + ", command: " + removePasswordFromDebugString(getCommand()));
      }
    } finally {
  ...
  protected final void preExecute() throws IOException, AgentFailureException {
    ...
    if (LOG.isDebugEnabled() && StringUtils.systemPropertyEquals("parabuild.hgdd.enabled", "true")) {
  ...
  private void validateRemoteCurrentDir() throws IOException {
    if (StringUtils.isBlank(remoteCurrentDir)) {
      throw new IOException("Error accessing Mercurial: Current directory is undefined.");
    }
  }
