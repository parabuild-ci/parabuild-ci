...
public final class P4SourceControl extends AbstractSourceControl implements CommonConstants {
  ...
  public void reloadConfiguration() {
    try {
      ...
      final int depotViewSourceCode = cm.getSourceControlSettingValue(buildID, SourceControlSetting.P4_CLIENT_VIEW_SOURCE, SourceControlSetting.P4_CLIENT_VIEW_SOURCE_VALUE_FIELD);
      if (depotViewSourceCode == SourceControlSetting.P4_CLIENT_VIEW_SOURCE_VALUE_DEPOT_PATH) {
        loginIfNecessary(getCheckoutDirectoryAwareAgent());
        updateDepotViewFromDepot(cm.getSourceControlSettingValue(buildID, SourceControlSetting.P4_CLIENT_VIEW_BY_DEPOT_PATH, null));
      } else if (depotViewSourceCode == SourceControlSetting.P4_CLIENT_VIEW_SOURCE_VALUE_CLIENT_NAME) {
        loginIfNecessary(getCheckoutDirectoryAwareAgent());
        updateDepotViewFromNamedWorkspace(cm.getSourceControlSettingValue(buildID, SourceControlSetting.P4_CLIENT_VIEW_BY_CLIENT_NAME, null));
      }
      ...
      hasToCleanUp |= scd.settingHasChanged(SourceControlSetting.P4_DEPOT_PATH);
      hasToCleanUp |= scd.settingHasChanged(SourceControlSetting.P4_CLIENT_VIEW_BY_DEPOT_PATH);
      hasToCleanUp |= scd.settingHasChanged(SourceControlSetting.P4_CLIENT_VIEW_SOURCE);
      hasToCleanUp |= scd.settingHasChanged(SourceControlSetting.VCS_CUSTOM_CHECKOUT_DIR_TEMPLATE);
      hasToCleanUp |= scd.settingHasChanged(SourceControlSetting.DO_NOT_CHECKOUT);
      if (hasToCleanUp) {
        setHasToCleanUp();
      }
      if (log.isDebugEnabled()) {
  ...
  private void loginIfNecessary(final Agent agent) throws CommandStoppedException, BuildException, AgentFailureException {
    final P4Properties props = getP4Properties();
    if (props.getAuthenticationMode() == SourceControlSetting.P4_AUTHENTICATION_MODE_VALUE_P4LOGIN) {
      InputStream is = null;
      P4Command command = null;
      try {
  ...
  private void lazyInitDepotViewFromDepotIfNecessary() throws IOException, CommandStoppedException, AgentFailureException {
    ...
    if (props.getClientViewSource() == SourceControlSetting.P4_CLIENT_VIEW_SOURCE_VALUE_FIELD) {
    ...
    } else if (props.getClientViewSource() == SourceControlSetting.P4_CLIENT_VIEW_SOURCE_VALUE_DEPOT_PATH) {
      ...
      lazyDepotInitCalled = true;
    } else if (props.getClientViewSource() == SourceControlSetting.P4_CLIENT_VIEW_SOURCE_VALUE_CLIENT_NAME) {
  ...
  private void updateDepotViewFromDepot(final String clientViewByDepotPath) throws IOException, CommandStoppedException, AgentFailureException {
    try {
      ...
      final String depotSourceClientViewClientSpec = makeClientSpec(agent, activeBuildID,
              ...
              SourceControlSetting.P4_LINE_END_VALUE_LOCAL);
