...
final class PVCSSourceControl extends AbstractSourceControl {
  ...
  public void reloadConfiguration() {
    ...
    hasToCleanUp |= scd.settingHasChanged(SourceControlSetting.PVCS_BRANCH_NAME);
//    hasToCleanUp = hasToCleanUp || scd.settingHasChanged(SourceControlSetting.PVCS_LABEL);
    hasToCleanUp |= scd.settingHasChanged(SourceControlSetting.PVCS_PROJECT);
    hasToCleanUp |= scd.settingHasChanged(SourceControlSetting.PVCS_PROMOTION_GROUP);
    hasToCleanUp |= scd.settingHasChanged(SourceControlSetting.PVCS_REPOSITORY);
  ...
  private void setCommonParameters(final PVCSCommandParameters parameters) {
    parameters.setBranch(getSettingValue(SourceControlSetting.PVCS_BRANCH_NAME));
    parameters.setPathToClient(getSettingValue(SourceControlSetting.PVCS_EXE_PATH));
    parameters.setRepository(getSettingValue(SourceControlSetting.PVCS_REPOSITORY));
    parameters.setPassword(StringUtils.isBlank(getSettingValue(SourceControlSetting.PVCS_PASSWORD)) ? "" : SecurityManager.decryptPassword(getSettingValue(SourceControlSetting.PVCS_PASSWORD)));
    parameters.setUser(getSettingValue(SourceControlSetting.PVCS_USER));
    parameters.setLabel(getSettingValue(SourceControlSetting.PVCS_LABEL));
    parameters.setPromotionGroup(getSettingValue(SourceControlSetting.PVCS_PROMOTION_GROUP));
  }
