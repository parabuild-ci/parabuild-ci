...
public final class MercurialManualScheduleStartParametersPanel extends ManualScheduleStartParametersPanel {
  ...
  public void load(final int buildID) {
    ...
    for (final Iterator i = sourceControlSettings.iterator(); i.hasNext();) {
      ...
      if ((setting.getPropertyName().equals(VersionControlSystem.MERCURIAL_BRANCH) || setting.getPropertyName().equals(VersionControlSystem.MERCURIAL_URL)) && !showURLOverride) {
        continue;
      }
  ...
  public boolean validate() {
    ...
    if (showURLOverride) {
      InputValidator.validateFieldNotBlank(errors, CAPTION_URL, flURL);
    }
